...
#         # c.execute("SELECT * FROM details where iddetails = " + str(Id))

#         #myresult = mycursor.fetchall()
#         # c = sqlite3.connect('oopcp.db')
#         # c.execute("SELECT * FROM details WHERE id = "+ str(Id) )
#         # r = c.fetchall()
...
def face_recognition():
    ...
    while True:
        ...
        for(x,y,w,h) in faces:
            ...
            #fetchdata(Id)
            # if(Id == 1):
            #     Id = "Atharva {0:.2f}%".format(round(100 - confidence, 2))
            # elif(Id == 2):
            #     Id = "Siddhi {0:.2f}%".format(round(100 - confidence, 2))
            # elif(Id == 3):
            #     Id = "Obama {0:.2f}%".format(round(100 - confidence, 2))
            # else:
